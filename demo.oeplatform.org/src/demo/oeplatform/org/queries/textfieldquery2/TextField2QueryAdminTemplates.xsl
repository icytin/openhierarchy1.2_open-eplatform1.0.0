...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<xsl:template match="ShowTextFieldQuery">
		...
		<h1><xsl:value-of select="$i18n.UpdateQuery" /><xsl:text>:&#160;</xsl:text><xsl:value-of select="TextField2Query/QueryDescriptor/name" /></h1>
		
		<fieldset>
			<legend><xsl:value-of select="$i18n.BaseInfo"/></legend>
			
			<div class="floatright">
				<a href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/updatequery/{TextField2Query/queryID}" title="{$i18n.UpdateBaseInformation}">
					<img class="alignbottom" src="{$imagePath}/pen.png"/>
				</a>
			</div>
			
			<xsl:call-template name="createCommonShowFields">
				<xsl:with-param name="element" select="TextField2Query" />
			</xsl:call-template>
			
			<xsl:variable name="layout" select="TextField2Query/layout" />
		...
		<fieldset>
			...
			<xsl:if test="TextField2Query/Fields/TextField2">
				
				<div class="floatleft full">
				
					<xsl:apply-templates select="TextField2Query/Fields/TextField2" mode="list" />
			...
			<xsl:if test="TextField2Query/Fields/TextField2">
				...
				<div class="floatright">
					<a href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/sorttextfields/{TextField2Query/queryID}" title="{$i18n.SortTextFields.Title}">
			...
			<div class="clearboth floatright">
				<a href="{/Document/requestinfo/currentURI}/{/Document/module/alias}/addtextfield/{TextField2Query/queryID}" title="{$i18n.AddTextField}">
	...
	<xsl:template match="TextField2" mode="list">
	...
	<xsl:template match="UpdateTextField">
		...
		<h1><xsl:value-of select="$i18n.UpdateTextField" /><xsl:text>:&#160;</xsl:text><xsl:value-of select="TextField2/label" /></h1>
		...
		<form id="udpateTextFieldForm" name="queryAdminForm" method="post" action="{/Document/requestinfo/uri}">
			...
			<xsl:call-template name="showTextFieldForm">
				<xsl:with-param name="element" select="TextField2" />
			</xsl:call-template>
			...
			<div class="floatleft full bigmarginbottom">
				...
				<div class="floatleft full">
					<xsl:call-template name="createDropdown">
						...
						<xsl:with-param name="selectedValue" select="TextField2Query/layout" />
					</xsl:call-template>
			    </div>
			</div>
	...
	<xsl:template match="SortTextFields">
		...
		<h1><xsl:value-of select="$i18n.SortTextFields" /><xsl:text>:&#160;</xsl:text><xsl:value-of select="TextField2Query/QueryDescriptor/name" /></h1>
		...
		<form id="sortTextFieldsForm" name="queryAdminForm" method="post" action="{/Document/requestinfo/uri}">
			...
			<div class="floatleft full sortable">
				...
				<xsl:apply-templates select="TextField2Query/Fields/TextField2" mode="sort" />
	...
	<xsl:template match="TextField2" mode="sort">
