...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
	...
	<div>Så här visas frågan för användaren</div>
	...
	<xsl:template match="ShowQueryValues">
		...
		<div class="query">
			...
			<article>
				...
				<div class="heading-wrapper">
					<h2>
						<xsl:attribute name="class">
							<xsl:if test="TextField2QueryInstance/QueryInstanceDescriptor/queryState = 'VISIBLE_REQUIRED'">
								<xsl:text>required</xsl:text>
							</xsl:if>
						</xsl:attribute>
						<xsl:value-of select="TextField2QueryInstance/QueryInstanceDescriptor/QueryDescriptor/name"/>
					</h2>
					
					<xsl:call-template name="createUpdateButton">
						<xsl:with-param name="queryID" select="TextField2QueryInstance/TextField2Query/queryID" />
					</xsl:call-template>
					
					<xsl:if test="TextField2QueryInstance/TextField2Query/description">
						<span class="italic">
							...
							<xsl:value-of select="TextField2QueryInstance/TextField2Query/description" disable-output-escaping="yes" />
						</span>		
					</xsl:if>
				...
				<fieldset>
					<xsl:apply-templates select="TextField2QueryInstance/TextField2Query/Fields/TextField2" mode="show"/>
				</fieldset>
	...
	<xsl:template match="ShowQueryForm">
		...
		<xsl:variable name="queryID" select="concat('query_', TextField2QueryInstance/TextField2Query/queryID)" />
		...
		<div class="query" id="{$queryID}">
			...
			<article>
				...
				<div class="heading-wrapper">
					...
					<xsl:if test="TextField2QueryInstance/TextField2Query/helpText">		
						<xsl:apply-templates select="TextField2QueryInstance/TextField2Query/helpText" />
					</xsl:if>
				...
				<fieldset>		
					<xsl:apply-templates select="TextField2QueryInstance/TextField2Query/Fields/TextField2" />
				</fieldset>
		...
		<script type="text/javascript">$(document).ready(function(){initTextFieldQuery('<xsl:value-of select="TextField2QueryInstance/TextField2Query/queryID" />');});</script>
		
	</xsl:template>
	
	<xsl:template match="TextField2">
	...
	<xsl:template match="TextField2" mode="show">
